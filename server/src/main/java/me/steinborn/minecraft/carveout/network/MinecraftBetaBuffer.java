package me.steinborn.minecraft.carveout.network;
...
/**
 * A wrapper for {@link ByteBuf} which includes methods for working with the "classic" Minecraft 1.7.3 protocol.
 */
public class MinecraftBetaBuffer extends ByteBuf {
    private final ByteBuf buf;

    public MinecraftBetaBuffer(ByteBuf buf) {
        this.buf = buf;
    }

    public String readUTF16String(int max) {
        short length = readShort();
        Preconditions.checkState(length <= max, "Trying to read too-long string (got %s, max %s)", length, max);
        if (!isReadable(length * 2)) {
            throw new IndexOutOfBoundsException(String.format(
                    "readerIndex(%d) + length(%d) exceeds writerIndex(%d): %s",
                    readerIndex(), length * 2, writerIndex(), this));
        }
        Preconditions.checkState(isReadable(length * 2), "Can't read " + length + " bytes from this packet");
        String str = toString(readerIndex(), length * 2, StandardCharsets.UTF_16BE);
        buf.readerIndex(buf.readerIndex() + (length * 2));
        return str;
    }
    ...
    //
    // Below this point, we're simply delegating to the backing buffer.
    //
    @Override
    public int capacity() {
        return buf.capacity();
    ...
    public String toString() {
        return "MinecraftBetaBuffer(" + buf.toString() + ")";
    }
